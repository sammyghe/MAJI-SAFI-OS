  ...
  return (
    <div className="border-b border-slate-200/50 last:border-0">
      <button
        ...
        className="w-full flex items-center gap-3 px-4 py-3 hover:bg-slate-100/30 transition-colors text-left"
      >
        <span className={`text-[10px] font-black px-2 py-0.5 rounded uppercase tracking-widest shrink-0 ${OP_COLOR[entry.operation]}`}>
          {entry.operation}
        </span>
        <span className="text-xs font-mono text-slate-400 shrink-0 w-36 truncate">{entry.table_name}</span>
        <span className="text-xs text-slate-600 font-mono truncate flex-1">{entry.row_id?.slice(0, 8)}…</span>
        <span className="text-[10px] text-slate-600 shrink-0">
          {new Date(entry.created_at).toLocaleString('en-GB', { dateStyle: 'short', timeStyle: 'short' })}
        </span>
        {hasDiff && (expanded ? <ChevronDown className="w-3 h-3 text-slate-600 shrink-0" /> : <ChevronRight className="w-3 h-3 text-slate-600 shrink-0" />)}
      </button>
      ...
      {expanded && !hasDiff && entry.new_data && (
        <div className="px-4 pb-3">
          <pre className="text-[10px] font-mono text-slate-500 whitespace-pre-wrap break-all max-h-32 overflow-y-auto">
    ...
    <div className="px-6 py-6 max-w-5xl mx-auto space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-slate-500 text-xs uppercase tracking-widest mt-1">Every change · Immutable · Trigger-written</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs text-slate-600">{total.toLocaleString()} entries</span>
        </div>
      </div>

      <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden">
        {loading ? (
          <p className="px-4 py-8 text-slate-600 text-xs font-black uppercase tracking-widest animate-pulse">Loading audit log…</p>
        ) : entries.length === 0 ? (
          <p className="px-4 py-8 text-slate-700 text-sm text-center">
            {tableFilter ? `No audit entries for table "${tableFilter}" yet.` : 'No audit entries yet. Entries appear after the first DB write.'}
          </p>
        ) : (
          <>
            <div className="px-4 py-2.5 border-b border-slate-200 grid grid-cols-4 gap-3">
              <p className="text-[10px] font-black text-slate-600 uppercase tracking-widest">Op</p>
              <p className="text-[10px] font-black text-slate-600 uppercase tracking-widest">Table</p>
              <p className="text-[10px] font-black text-slate-600 uppercase tracking-widest">Row</p>
              <p className="text-[10px] font-black text-slate-600 uppercase tracking-widest text-right">Time</p>
            </div>
      ...
      {total > PAGE_SIZE && (
        <div className="flex items-center justify-center gap-4">
          <button disabled={page === 0} onClick={() => setPage(p => p - 1)}
            className="px-4 py-2 bg-slate-100 border border-slate-200 rounded-lg text-xs font-black text-slate-400 hover:text-white disabled:opacity-30">
            ← Prev
          </button>
