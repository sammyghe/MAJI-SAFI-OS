...
const STATUS_CONFIG: Record<string, { icon: React.ComponentType<{ className?: string }>; color: string; bg: string; label: string }> = {
  ...
  not_started: { icon: Minus,      color: 'text-slate-500',   bg: 'bg-slate-100',        label: 'Not Started' },
};
  ...
  return (
    <div className="px-6 py-6 max-w-5xl mx-auto space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        {versions.length > 1 && (
          <select className="bg-slate-100 border border-slate-200 rounded-lg px-3 py-2 text-xs text-white focus:outline-none">
      ...
      <div className="flex flex-wrap gap-2">
        <button onClick={() => setCategoryFilter('all')}
          className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-colors ${categoryFilter === 'all' ? 'bg-[#0077B6]/20 text-[#7EC8E3] border border-[#0077B6]/30' : 'bg-slate-100 text-slate-500 border border-slate-200 hover:text-slate-300'}`}>
          All
        </button>
        {CATEGORIES.filter(c => assumptions.some(a => a.category === c)).map(c => (
          <button key={c} onClick={() => setCategoryFilter(c)}
            className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-colors ${categoryFilter === c ? 'bg-[#0077B6]/20 text-[#7EC8E3] border border-[#0077B6]/30' : 'bg-slate-100 text-slate-500 border border-slate-200 hover:text-slate-300'}`}>
            {CAT_LABELS[c]}
          </button>
        ))}
      </div>
      ...
      {loading ? (
        <p className="text-slate-600 text-xs font-black uppercase tracking-widest animate-pulse">Loading plan…</p>
      ) : (
          ...
          {Object.entries(grouped).map(([cat, items]) => (
            <div key={cat} className="bg-white border border-slate-200 rounded-2xl overflow-hidden">
              <div className="px-5 py-3 border-b border-slate-200 bg-slate-100/30">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">{CAT_LABELS[cat]}</p>
              </div>
              <div>
                {items.map((a, i) => (
                  <div key={a.id} className={`${i < items.length - 1 ? 'border-b border-slate-200/50' : ''}`}>
                    <div className="px-5 py-4 flex items-start gap-4">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-slate-400 leading-relaxed">{a.assumption_text}</p>
                        {(a.target_value !== null || a.actual_value !== null) && (
                          <div className="flex items-center gap-4 mt-2">
                            {a.target_value !== null && (
                              <div className="flex items-center gap-1.5">
                                <span className="text-[9px] text-slate-600 uppercase font-black">Target</span>
                                <span className="text-xs font-black text-slate-400">{formatValue(a.target_value, a.unit)}</span>
                              </div>
                            )}
                            {a.actual_value !== null && (
                              <div className="flex items-center gap-1.5">
                                <span className="text-[9px] text-slate-600 uppercase font-black">Actual</span>
                        ...
                        {a.notes && <p className="text-[11px] text-slate-600 mt-1.5 italic">{a.notes}</p>}
                        {a.source_table && (
                          <p className="text-[9px] text-slate-700 mt-1">[source: {a.source_table}]</p>
                        )}
                      </div>
                      {isFounder && editingId !== a.id && (
                        <button onClick={() => startEdit(a)} className="shrink-0 text-slate-700 hover:text-slate-400 transition-colors mt-0.5">
                    ...
                    {editingId === a.id && (
                      <div className="px-5 pb-4 bg-slate-100/30 border-t border-slate-200/50">
                        <div className="flex flex-wrap gap-3 pt-3">
                          ...
                          <input value={editForm.actual_value} onChange={e => setEditForm({ ...editForm, actual_value: e.target.value })}
                            ...
                            className="w-40 bg-slate-100 border border-slate-200 rounded-lg px-3 py-2 text-xs text-white focus:outline-none" />
                          <input value={editForm.notes} onChange={e => setEditForm({ ...editForm, notes: e.target.value })}
                            placeholder="Notes"
                            className="flex-1 min-w-40 bg-slate-100 border border-slate-200 rounded-lg px-3 py-2 text-xs text-white focus:outline-none" />
      ...
      {activeVersion?.summary && (
        <p className="text-[10px] text-slate-700">{activeVersion.summary}</p>
      )}
    </div>
  );
}
