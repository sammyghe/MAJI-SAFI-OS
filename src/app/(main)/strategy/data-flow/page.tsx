...
const SHARE_TYPE_STYLE: Record<string, { cell: string; badge: string; label: string }> = {
  ...
  none:    { cell: 'bg-zinc-800/30', badge: 'bg-zinc-800 text-zinc-600',                                 label: 'None' },
};
...
export default function DataFlowPage() {
  ...
  return (
    <div className="px-6 py-6 max-w-full mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={exportCSV}
            className="flex items-center gap-2 px-4 py-2 bg-zinc-800 border border-zinc-700 rounded-xl text-xs font-black text-zinc-400 hover:text-white">
      ...
      {/* Add flow form */}
      {showAdd && (
        <div className="bg-zinc-900 border border-[#0077B6]/30 rounded-2xl p-5 space-y-3">
          <p className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">New Data Flow</p>
          <div className="flex flex-wrap gap-3">
            <div>
              <p className="text-[9px] text-zinc-600 uppercase font-black mb-1">Source Dept</p>
            ...
            <div>
              <p className="text-[9px] text-zinc-600 uppercase font-black mb-1">Target Dept</p>
            ...
            <div className="flex-1 min-w-40">
              <p className="text-[9px] text-zinc-600 uppercase font-black mb-1">Data Category</p>
            ...
            <div>
              <p className="text-[9px] text-zinc-600 uppercase font-black mb-1">Share Type</p>
            ...
            <div className="flex-1 min-w-40">
              <p className="text-[9px] text-zinc-600 uppercase font-black mb-1">Entity Table</p>
          ...
          <div className="flex gap-3">
            ...
            <button onClick={() => setShowAdd(false)}><X className="w-4 h-4 text-zinc-500" /></button>
          </div>
        </div>
      )}
      ...
      {/* Matrix view */}
      {loading ? (
        <p className="text-zinc-600 text-xs font-black uppercase tracking-widest animate-pulse">Loading matrix…</p>
      ) : (
        <div className="overflow-x-auto">
          <table className="min-w-max text-xs border-collapse">
            <thead>
              <tr>
                <th className="text-left px-3 py-2 text-[10px] text-zinc-600 font-black uppercase tracking-widest w-28 border-b border-zinc-800">
                  Source ↓ / Target →
                </th>
                {DEPTS.map(target => (
                  <th key={target} className="px-2 py-2 text-center border-b border-zinc-800 min-w-[90px]">
            ...
            <tbody>
              {DEPTS.map(source => (
                <tr key={source} className="border-b border-zinc-900">
                  <td className="px-3 py-2 text-[10px] font-black text-zinc-400 uppercase tracking-widest whitespace-nowrap border-r border-zinc-800">
                    {DEPT_LABELS[source]}
                  </td>
                  {DEPTS.map(target => {
                    if (source === target) {
                      return <td key={target} className="px-2 py-1 text-center bg-zinc-900/50"><span className="text-zinc-800">—</span></td>;
                    }
                    ...
                    return (
                      <td key={target} className="px-2 py-1">
                        ...
                        >
                          <span className={`text-[9px] font-black uppercase block truncate ${topRel.active ? '' : 'text-zinc-600'}`}>
                            {topRel.data_category.replace(/_/g, ' ')}
                          </span>
                          {activeRels.length > 1 && (
                            <span className="text-[8px] text-zinc-600">+{activeRels.length - 1}</span>
                          )}
                        </button>
                      </td>
                    );
      ...
      <div className="flex flex-wrap gap-4 text-[10px] text-zinc-600">
        ...
        <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-zinc-800/30" />None / inactive</span>
      </div>
      ...
      {selected && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={() => { setSelected(null); setEditing(false); }}>
          <div className="bg-zinc-900 border border-zinc-700 rounded-2xl w-full max-w-lg p-6 space-y-4" onClick={e => e.stopPropagation()}>
            <div className="flex items-start justify-between">
              ...
              <button onClick={() => { setSelected(null); setEditing(false); }}><X className="w-5 h-5 text-zinc-500" /></button>
            </div>

            <div className="flex items-center gap-3 flex-wrap">
              <ShareTypeBadge type={selected.share_type} />
              {selected.entity_table && (
                <span className="text-[10px] font-mono text-zinc-500 bg-zinc-800 px-2 py-1 rounded">{selected.entity_table}</span>
              )}
              <span className="text-[10px] text-zinc-600">{selected.refresh_frequency === 'realtime' ? '⚡ Live' : `↻ ${selected.refresh_frequency}`}</span>
              <span className={`text-[10px] font-black ${selected.active ? 'text-emerald-400' : 'text-zinc-600'}`}>
            ...
            {editing ? (
              <div className="space-y-3">
                <div>
                  <p className="text-[10px] text-zinc-500 uppercase font-black mb-1">Why shared</p>
                  <textarea value={editForm.why_shared} onChange={e => setEditForm({...editForm, why_shared: e.target.value})}
                    rows={3}
                    className="w-full bg-zinc-800 border border-zinc-700 rounded-xl px-3 py-2 text-xs text-white resize-none focus:outline-none focus:border-[#0077B6]/50" />
                </div>
                <div className="flex gap-3">
              ...
              <div className="space-y-3">
                <div className="bg-zinc-800/50 rounded-xl p-4">
                  ...
                  <p className="text-sm text-zinc-300 leading-relaxed">{selected.why_shared}</p>
                </div>
                {isFounder && (
                  <div className="flex gap-3">
                    <button onClick={() => setEditing(true)}
                      className="flex items-center gap-1.5 px-4 py-2 bg-zinc-800 border border-zinc-700 rounded-lg text-xs font-black text-zinc-400 hover:text-white">
                      <Edit2 className="w-3 h-3" /> Edit
                    </button>
                    <button onClick={() => toggleActive(selected)}
                      className={`flex items-center gap-1.5 px-4 py-2 border rounded-lg text-xs font-black ${selected.active ? 'bg-zinc-800 border-zinc-700 text-zinc-400 hover:text-red-400' : 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400 hover:bg-emerald-500/20'}`}>
